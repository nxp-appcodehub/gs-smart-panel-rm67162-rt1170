 ...
 * Variables
 ******************************************************************************/
#if DEMO_RM67162_USE_DSI_AUX
static dsi_aux_handle_t s_dsiDriverHandle;
// static uint8_t *s_dsiMemWriteTmpArray[480000];
#else
...
#else
static void BOARD_DsiMemWriteCallback(const MIPI_DSI_Type *base, dsi_aux_handle_t *handle, status_t status, void *userData)
{
    MIPI_DSI_MemoryDoneDriverCallback(status, &dsiDevice);
}
#endif /* DEMO_RM67162_USE_DSI_AUX */
...
static status_t BOARD_DSI_MemWrite(uint8_t virtualChannel, const uint8_t *data, uint32_t length)
{
...
#if (DEMO_RM67162_BUFFER_FORMAT == DEMO_RM67162_BUFFER_RGB565)
        .inputFormat  = kDSI_AUX_InputPixelFormatRGB565,
        .outputFormat = kDSI_AUX_OutputPixelFormatRGB565,
#elif (DEMO_RM67162_BUFFER_FORMAT == DEMO_RM67162_BUFFER_RGB888)
        .inputFormat  = kDSI_AUX_InputPixelFormatRGB888,
        .outputFormat = kDSI_AUX_OutputPixelFormatRGB888,
#else
        .inputFormat  = kDSI_AUX_InputPixelFormatXRGB8888,
        .outputFormat = kDSI_AUX_OutputPixelFormatRGB888,
    ...
    return DSI_TransferWriteMemoryAUX(DEMO_MIPI_DSI, &s_dsiDriverHandle, &xfer);
#else /* DEMO_RM67162_USE_DSI_AUX */
    status_t status;
...
status_t BOARD_PrepareDisplayController(void)
{
    ...
    if (kStatus_Success == status)
    {
        status = DSI_TransferCreateHandleAUX(DEMO_MIPI_DSI, &s_dsiDriverHandle, BOARD_DsiMemWriteCallback, NULL);
    }
